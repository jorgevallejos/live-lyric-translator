...
| `videoRunsBroadcast.ts` | localStorage | **Whether the video runs tonight**, as the drive mode answered it. The wall knows the room's assignment and nothing about what was chosen, so it is told the answer rather than the inputs. Replaced the 3-way display-mode broadcast, in which only `none` versus not-`none` ever did anything |
...
| `gigPhase.ts` | — | **The gig's three states during performance**: `before` the setlist, `during` it, `after` it. `after` is decided by the played log alone, which only appends — so the last song ending closes the setlist and nothing reopens it, and unarming goes back rather than forward. Moments 3, 9, 11 and 12 all attach here |
| `gigContactState.ts` | localStorage | **The message home's one condition** — outside the setlist, unless a song is actually being performed — and the boolean it travels as |
...
| `messageHomePrefs.ts` | localStorage | **The message home's four values, as this machine remembers them.** Artist-level, so later gigs arrive prefilled. **Not where the player finds them** — the content travels in the gig, because a player that reads only the gig folder cannot read the shell's Preferences |
| `gigFile.ts` | — | `gig.json` — parse, serialise, create, and project the setlist into it, including the `messageHome` block. Pregonero is its only writer. Also **a gig's identity**: `newGigId` (the folder is an opaque id, never derived and never changed), `gigIdentityIsAnswered` (nothing is written until the date and the venue are both answered) and `gigLabel` (what a list calls it) |
...
**Video mode is two video elements, transport-synced — not a shared clock.** The audience output is `ShapeVideo.tsx`, mounted inside the gig's `song-video` shape: **it is the clock**, reporting its own `currentTime` up so the lyric lands in the `song-lyrics` shape — two shapes, not one frame with an overlay. It mounts **paused at `trimStart`** and obeys `play` / `pause` / `seek` commands broadcast from the Control window's `VideoPerformancePanel.tsx` over the `localStorage` transport channel (`videoTransport.ts`). The performer panel runs the single-clock count-in and, at the count-in→video handoff (`beginFired`), broadcasts `play` so the audience video starts on the downbeat; Pause broadcasts `pause`; Restart broadcasts `seek(trimStart)` then `play` at the next handoff. The panel also keeps a local preview `<video>` for the performer. Because the two elements aren't continuously time-synced, drift is corrected by manual seek — not a periodic resync (a known trade-off). **`screenSizeState.ts` is gone entirely as of 2026-09-06.** Its `ScreenSize` half went on 2026-09-03, when a sweep traced it end to end and found no reader — it never reached the status text, the display profile it also wrote was never read back, and the WS `screenSize` message was relayed by the main process and handled by no receiver. **`DisplayMode` was what remained, and in the Projection window it reached exactly one expression: whether the video runs at all**, with `small` and `big` indistinguishable there.

**So it disappeared rather than collapsing to on-and-off** (Jorge, 2026-09-03): format and placement are Muralista's, decided at the wall; **whether the video runs tonight is the drive mode**; and **size was never a third thing** — it was format wearing a performance control's clothes. What crossed to the wall was always one boolean wearing three names, and it is one now: `videoRunsBroadcast.ts`. What survived the file is the projection column's own two words, in `projectionStatus.ts`.
